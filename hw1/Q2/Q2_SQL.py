...
class HW2_sql():
    ...
    def part_aii_2(self,connection, path):
        ############### CREATE IMPORT CODE BELOW ############################
        with open('data/movie_cast.csv', newline='', encoding='utf-8') as file:
            data = csv.reader(file, delimiter=',')
    ...
    def part_aiii(self,connection):
        ...
        part_aiii_sql = "CREATE TABLE cast_bio(cast_id INTERGER PRIMARY KEY, cast_name TEXT, birthday TEXT, popularity REAL);"
        ######################################################################
    ...
    def part_c(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_c_sql = "SELECT  printf('%.2f', count(*) * 100.0 / (SELECT COUNT(*)FROM movies))FROM movies WHERE (score > 50) AND (LOWER(title) LIKE '%war%')"
        ######################################################################
    ...
    def part_d(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_d_sql = "SELECT cast_name,  COUNT(movie_id) AS appearance_count FROM movie_cast WHERE popularity > 10 GROUP BY  cast_name ORDER BY appearance_count DESC, cast_name ASC LIMIT 5"
        ######################################################################
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = "SELECT movies.title AS movie_title, printf ('%.2f', movies.score) AS movie_score, count(movie_cast.cast_name) AS cast_count FROM movies INNER JOIN movie_cast on movies.id = movie_cast.movie_id GROUP BY movie_title ORDER BY movie_score+0 DESC, cast_count ASC, movie_title ASC LIMIT 5"
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = "SELECT movie_cast.cast_id, movie_cast.cast_name, printf ('%.2f', avg(movies.score)) AS average_score FROM movies INNER JOIN movie_cast on movies.id = movie_cast.movie_id WHERE(movies.score>= 25) GROUP BY cast_name HAVING count(*) >2 ORDER BY average_score+0 DESC, cast_name ASC LIMIT 10"
        ######################################################################
    ...
    def part_g(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_sql = "CREATE VIEW good_collaboration AS SELECT c1.cast_id AS cast_member_id1, c1.cast_name AS cast_name1, c2.cast_id AS cast_member_id2, c2.cast_name AS cast_name2, count(m.id) AS movie_count, avg(m.score) AS average_movie_score FROM movie_cast AS c1 JOIN movie_cast AS c2 ON c1.movie_id = c2.movie_id AND c1.cast_id < c2.cast_id JOIN movies AS m ON c1.movie_id = m.id GROUP BY cast_member_id1, cast_member_id2 HAVING movie_count >= 3 and average_movie_score >= 40;"
        ######################################################################
        return self.execute_query(connection, part_g_sql)
    
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = "SELECT cast_id, cast_name, printf('%.2f', sum((movie_count)*(average_movie_score))/sum(movie_count)) AS collaboration_score FROM (SELECT * FROM (SELECT cast_member_id1 as cast_id, cast_name1 as cast_name, movie_count, average_movie_score FROM good_collaboration) UNION ALL SELECT * FROM (SELECT cast_member_id2 as cast_id, cast_name2 as cast_name, movie_count, average_movie_score FROM good_collaboration)) GROUP BY cast_id ORDER BY collaboration_score DESC, cast_name ASC LIMIT 5"
        ######################################################################
    ...
    def part_h(self,connection,path):
        ############### EDIT SQL STATEMENT ###################################
        part_h_sql = "CREATE VIRTUAL TABLE movie_overview USING fts3(id INTERGER PRIMARY KEY, overview TEXT)"
        ######################################################################
        connection.execute(part_h_sql)
        ############### CREATE IMPORT CODE BELOW ############################
        with open('data/movie_overview.csv', newline='', encoding='utf-8') as file:
            data = csv.reader(file, delimiter=',')
    ...
    def part_hi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hi_sql = "SELECT COUNT(*) FROM (SELECT overview FROM movie_overview WHERE overview MATCH '^fight') ;"
        ######################################################################
    ...
    def part_hii(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_hii_sql = "SELECT COUNT(*) FROM (SELECT overview FROM movie_overview WHERE overview MATCH 'space NEAR/5 program')"
        ######################################################################
