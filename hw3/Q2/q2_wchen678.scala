...
// ENTER THE CODE BELOW
val df1 = df_filter.groupBy("DOLocationID").agg(count("DOLocationID") as "number_of_dropoffs").orderBy(desc("number_of_dropoffs"), asc("DOLocationID")).withColumn("number_of_dropoffs",col("number_of_dropoffs").cast("int"))
...
// ENTER THE CODE BELOW
val df2 = df_filter.groupBy("PULocationID").agg(count("PULocationID") as "number_of_pickups").orderBy(desc("number_of_pickups"), asc("PULocationID")).withColumn("number_of_pickups",col("number_of_pickups").cast("int"))
df2.limit(5).show()
...
// ENTER THE CODE BELOW
val df3 = df1.join(df2,df1("DOLocationID") === df2("PULocationID"),"outer").withColumn("number_activities",col("number_of_pickups")+col("number_of_dropoffs")).withColumnRenamed("DOLocationID","LocationID").select("LocationID", "number_activities").orderBy(desc("number_activities"))
df3.limit(3).show()
...
// ENTER THE CODE BELOW
val df4 = df3.join(df_taxi,df3("LocationID") === df_taxi("LocationID")).groupBy("Borough").agg(sum("number_activities") as "total_number_activities").orderBy(desc("total_number_activities")).withColumn("total_number_activities",col("total_number_activities").cast("int"))
...
// ENTER THE CODE BELOW
val df5 = df_filter.withColumn("day_of_week", date_format(col("pickup_datetime"), "EEEE")).withColumn("date", df("pickup_datetime").cast(DateType)).groupBy("day_of_week").agg(count("day_of_week")/countDistinct("date") as "avg_count").orderBy(desc("avg_count")).withColumn("avg_count",col("avg_count").cast("float"))
...
val df6 = df_filter.join(df_taxi,df_filter("PULocationID") === df_taxi("LocationID")).where("Borough = 'Brooklyn'").withColumn("hour_of_day", date_format(col("pickup_datetime"), "H")).withColumn("id",concat_ws(",",col("hour_of_day"),col("Zone")))

//val windowSpec  = Window.partitionBy("hour_of_day").orderBy()

//val df6_count = df6.withColumn("count", count("PULocationID").over(windowSpec))
//.withColumn("id",concat_ws(",",col("hour_of_day"),col("Zone")))
//.withColumn("hour_of_day",col("hour_of_day").cast("int"))

val df6_1 = df6.groupBy("id").agg(count("PULocationID").alias("count")).withColumnRenamed("id","id_grouped")
val df6_2 = df6_1.join(df6, df6_1("id_grouped") === df6("id"), "left").select("id", "count", "PULocationID", "hour_of_day", "Zone").dropDuplicates()

val df6_3 = df6_2.withColumn("count_zone",concat_ws(",",col("count"),col("Zone")))
val df6_4 = df6_3.groupBy("hour_of_day").agg(max("count").alias("max_count")).withColumnRenamed("hour_of_day","hour_of_day_max")

val df6_5 = df6_4.join(df6_2, (df6_4("hour_of_day_max") ===  df6_2("hour_of_day")) && (df6_4("max_count") ===  df6_2("count")), "inner")
.select("hour_of_day", "Zone", "max_count").withColumn("max_count",col("max_count").cast("int")).withColumn("hour_of_day",col("hour_of_day").cast("int")).sort("hour_of_day")
...
val df7 = df_filter.join(df_taxi,df_filter("PULocationID") === df_taxi("LocationID")).withColumn("date", date_format(col("pickup_datetime"), "yyyy-MM-dd")).where("Borough = 'Manhattan'").groupBy("date").agg(count("pickup_datetime").alias("count")).withColumn("day", date_format(col("date"), "D"))

val win = Window.partitionBy().orderBy("date")
val df7_1 = df7.withColumn("prev_count", lag(col("count"),1).over(win)).withColumn("percenatge_change", (col("count") - col("prev_count")) / col("prev_count") *100).filter(col("percenatge_change") > 0).orderBy(desc("percenatge_change"))

val df7_2 = df7_1.where(!$"date".contains("-01-01")).filter(col("date").like("%-01-%")).select("day", "percenatge_change").withColumn("day",col("day").cast("int"))
